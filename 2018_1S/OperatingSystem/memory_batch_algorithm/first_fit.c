...
void firstfit()
{
	...
	for (i = 0; i < NUMOFPROC; i++)
	{
		...
		if (p[i].load != true)	//i번 프로세스가 적재되어 있지 않을 경우 메모리 스캔 시작
		{
			...
			for (m = 0; m < SIZEOFMEM; m++)
			{
				...
				for (j = 0; j < p[i].size; j++)		//memory[x]부터 memory[x+p[i].size]까지가 모두 0일 경우 x번지에 할당
				{
					...
					if (j == p[i].size - 1)			//해당 메모리주소(x)부터 해당 프로세스p[i]크기만큼의 주소가 비어있다면 메모리 전체의 빈공간 모두 확인할때 까지 가장 최소의 크기를 찾는다.
					{
						...
						printf("%d번 프로세스의 firstfit 주소는 %d이고 크기는 %d 이다.\n\n", i + 1, m, mem[y].size);

						//이곳에서 p[i]를 firstfit에 맞는 메모리의 빈공간에 적재한다
						//이곳 구현한다.
						...
						printf("%d번 프로세스의 firstfit 주소는 %d이고 크기는 %d만큼 저장이 완료되었습니다.(적재 완료 여부 : %d)\n\n", i + 1, p[i].locate, p[i].size, p[i].load);
						break;
			...
			//메모리 부족 할 경우
			if (p[i].load != 1)//프로세스가 적재가 안됬을 경우
				printf("%d번 프로세스의 firstfit 주소는 %d이고 크기는 %d만큼 저장이 메모리 부족으로 실패되었습니다.(적재 완료 여부 : %d)\n\n", i + 1, p[i].locate, p[i].size, p[i].load);
