		...
		if (p[i].load != true)	//i번 프로세스가 적재되어 있지 않을 경우 메모리 스캔 시작
		{
			...
			for (k = 0; k < NUMOFPROC; k++)
			{
				if (mem[k].size != 0)
				{
					...
					if (mem[k].size < tmp_size) //k번쨰 가용메모리가 tmp보다 작을경우
					{
						tmp_size = mem[k].size;
			...
			{
				printf("%d번 프로세스의 bestfit 주소는 %d이고 크기는 %d 이다.\n\n", i + 1, tmp_locate, tmp_size);

				p[i].locate = tmp_size;

				//이곳에서 p[i]를 bestfit에 맞는 메모리의 빈공간에 적재한다
				//이곳 구현한다.
				...
				printf("%d번 프로세스의 bestfit 주소는 %d이고 크기는 %d만큼 저장이 완료되었습니다.(적재 완료 여부 : %d)\n\n", i + 1, p[i].locate, p[i].size, p[i].load);
			}
			else //메모리 부족 할 경우
			{
				printf("%d번 프로세스의 bestfit 주소는 %d이고 크기는 %d만큼 저장이 메모리 부족으로 실패되었습니다.(적재 완료 여부 : %d)\n\n", i + 1, p[i].locate, p[i].size, p[i].load);
			}
